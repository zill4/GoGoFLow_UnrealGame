...
void UOnlineGameInstance::OnLoginComplete(int ControllerIndex, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
{
	UE_LOG(LogTemp, Warning, TEXT("OnLoginComplete: %d"), bWasSuccessful);

	if (IOnlineIdentityPtr Identity = OnlineSubsystem->GetIdentityInterface())
	{
		Identity->ClearOnLoginCompleteDelegates(ControllerIndex, this);
	}

	if (Error.Contains(TEXT("already logged in")))
	{
		bIsLoggedIn = true;
		//LoginCompleteDelegate.Broadcast();
	...
	//LoginCompleteDelegate.Broadcast(Error);
}
...
{
	UE_LOG(LogTemp, Warning, TEXT("UOnlineGameInstace::OnLogoutComplete(): %d"), bWasSuccessful);

	bIsLoggedIn = !bWasSuccessful;

	if (IOnlineIdentityPtr Identity = OnlineSubsystem->GetIdentityInterface())
	{
		Identity->ClearOnLogoutCompleteDelegates(ControllerIndex, this);
	}

	//if (!bWasSuccessful)
	//{
		//LogoutCompleteDelegate.Broadcast();
	//}
	//else
	//{
		//LogoutCompleteDelegate.Broadcast();
...
void UOnlineGameInstance::OnFindSessionsComplete(bool bWasSuccess)
{
	...
	if (bWasSuccess && SessionSearch.IsValid())
	{
		...
		//SessionFoundCompleteDelegate.Broadcast(Results);

		return;
	}

	//SessionFoundCompleteDelegate.Broadcast();
}
